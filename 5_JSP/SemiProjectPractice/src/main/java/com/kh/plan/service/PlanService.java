...
//		System.out.println(conn);
	...
	public ArrayList<Plan> insertPlan2(String userId, String title, Date startDate, Date endDate, Date remindAlarmDate) {
		// 1) Connection 객체 생성
		Connection conn = JDBCTemplate2.getConnection();
//		System.out.println(conn);
		// 2) Dao 에게 데이터 전달과 함께 필요한 메소드 호출
		...
		if(result >0) {
			JDBCTemplate2.commit(conn);
			list = new PlanDao().selectPlan2(conn, userId);
		} else {
			JDBCTemplate2.rollback(conn);
		}
	...
	public ArrayList<Plan> insertPlan3(String userId, String title, Date startDate, Date endDate, Date remindAlarmDate) {
		// 1) Connection 객체 생성
		Connection conn = JDBCTemplate2.getConnection();
//		System.out.println(conn);
		// 2) Dao 에게 데이터 전달과 함께 필요한 메소드 호출
		...
		if(result >0) {
			JDBCTemplate2.commit(conn);
			list = new PlanDao().selectPlan3(conn, userId);
		} else {
			JDBCTemplate2.rollback(conn);
		}
	...
	public ArrayList<Plan> insertPlan4(String userId, String title, Date startDate, Date endDate, Date remindAlarmDate) {
		// 1) Connection 객체 생성
		Connection conn = JDBCTemplate2.getConnection();
//		System.out.println(conn);
		// 2) Dao 에게 데이터 전달과 함께 필요한 메소드 호출
		...
		if(result >0) {
			JDBCTemplate2.commit(conn);
			list = new PlanDao().selectPlan4(conn, userId);
		} else {
			JDBCTemplate2.rollback(conn);
		}
