...
<body>
	...
	<div class="outer">
		...
		<%-- if(loginUser != null && loginUser.getUserId().equals("admin")) { --%> <c:if test="${loginUser != null and loginUser.userId eq 'admin' }">
	...
	<table align="center" class="list-area">
		...
		<tbody>
			...
			<%-- for(Notice n : list) { --%> 	<c:forEach var="n" items="${ list }">
			<tr>
				<td><%--= n.getNoticeNo() --%>${n.noticeNo }</td>
				<td><%--= n.getNoticeTitle() --%>${n.noticeTitle }</td>
				<td><%--= n.getNoticeWriter() --%>${n.noticeWriter }</td>
				<td><%--= n.getCount() --%>${n.count }</td>
				<td><%--= n.getCreateDate() --%>${n.createDate }</td>
			</tr>
			<%-- } --%>
		<%-- } --%>
		</c:forEach>
